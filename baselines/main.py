...
from agents import Random_Agent, Lookahead_Agent
...
# ----------------------------------------------------------------
# 2) SIMULATOR FACTORY (headless—no new window)
# ----------------------------------------------------------------
def make_sim_env():
    sim = gym_super_mario_bros.make(
        ENV_NAME,
        render_mode=None,                # <<< NO WINDOW HERE
        apply_api_compatibility=True
    )
...
# 2) Create one “live” env for training:
env = live_env
